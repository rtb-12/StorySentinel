...
class YakoaService {
    constructor() {
        ...
        if (!this.baseUrl || !this.apiKey) {
            console.warn("Yakoa API configuration missing. Please set YAKOA_BASE_URL and YAKOA_API_KEY in your .env file");
        }
    }
    getHeaders() {
        return {
    ...
    async registerToken(tokenData) {
        ...
        console.log("Registering token with Yakoa:", JSON.stringify(tokenData, null, 2));
        try {
            const response = await (0, node_fetch_1.default)(`${this.baseUrl}/token`, {
                method: "POST",
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to register token with Yakoa: ${response.status} ${errorText}`);
            }
    ...
    async getToken(tokenId) {
        ...
        try {
            const response = await (0, node_fetch_1.default)(`${this.baseUrl}/token/${tokenId}`, {
                method: "GET",
                headers: this.getHeaders(),
            });
            if (!response.ok) {
                ...
                throw new Error(`Failed to fetch token from Yakoa: ${response.status} ${errorText}`);
            }
            const result = (await response.json());
     ...
     */
    createTokenRegistrationData(storyResult, ipMetadata, mediaUrls, creatorId, chain = "story-testnet") {
        ...
        if (formattedContractAddress.length !== 42 ||
        ...
        // Ensure it's exactly 42 characters (0x + 40 hex chars) and only contains valid hex
        if (formattedCreatorId.length !== 42 ||
     ...
     */
    generateTokenIdentifier(chain, contractAddress, tokenId) {
...
exports.yakoaService = new YakoaService();
exports.default = exports.yakoaService;
