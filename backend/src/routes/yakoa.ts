...
import { yakoaService } from "../services/yakoa";
import { YakoaTokenRegistrationRequest } from "../types";
...
router.post("/register", async (req: Request, res: Response) => {
  try {
    let registrationData: YakoaTokenRegistrationRequest = req.body;
    ...
    if (
      typeof registrationData.id === "string" &&
      ...
      if (parts.length >= 3) {
        ...
        console.log(
          `Auto-fixed ID format for registration: ${req.body.id} -> ${registrationData.id}`
        );
      }
    } else if (
      typeof registrationData.id === "string" &&
    ...
    if (!yakoaService.isConfigured()) {
      return res.status(503).json({
        error: "Yakoa API not configured",
        message:
          "Please configure YAKOA_BASE_URL and YAKOA_API_KEY environment variables",
        timestamp: new Date().toISOString(),
      });
    }

    const result = await yakoaService.registerToken(registrationData);
...
router.get("/token/:tokenId", async (req: Request, res: Response) => {
  try {
    ...
    const result = await yakoaService.getToken(tokenId);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        "Creating registration data for Story Protocol result:",
        storyResult
      );

      const registrationData = yakoaService.createTokenRegistrationData(
        storyResult,
        ipMetadata,
        mediaUrls,
        creatorId,
...
router.post("/generate-identifier", async (req: Request, res: Response) => {
  try {
    ...
    const identifier = yakoaService.generateTokenIdentifier(
      chain,
      contractAddress,
      tokenId
    );
...
router.get("/health", (req: Request, res: Response) => {
  const isConfigured = yakoaService.isConfigured();
  ...
  try {
    ...
    if (
      ...
      if (parts.length >= 3) {
        ...
        registrationData.id = `${address}:${tokenId}`;
        console.log(
          `Auto-fixed ID format: ${req.body.id} -> ${registrationData.id}`
        );
      }
    } else if (
      typeof registrationData.id === "string" &&
    ...
    if (
      ...
      validationErrors.push({
        field: "id",
        message:
          "Should match pattern ^0x[a-f0-9]{40}(:[0-9]+)?$ (case insensitive)",
    ...
    if (validationErrors.length > 0) {
      return res.status(400).json({
        ...
        data: registrationData,
        originalData:
          req.body.id !== registrationData.id ? req.body : undefined,
        timestamp: new Date().toISOString(),
      });
    }
    ...
    res.json({
      ...
      data: registrationData,
      autoFixed:
        req.body.id !== registrationData.id
          ? {
...
router.get("/test-data", async (req: Request, res: Response) => {
  try {
    ...
    const sampleData: YakoaTokenRegistrationRequest = {
